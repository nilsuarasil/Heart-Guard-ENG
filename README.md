...
- To install the required libraries:
  ...
  ```
  > `tensorflow` is only required for training the model from scratch.
  > The dashboard (`main.py`) runs without TensorFlow dependency.

### 📁 About Large Files (Important)

**`ptbxl_database.csv`** is not included in this repo (excluded via `.gitignore` due to licensing and size).
Download it from PhysioNet for training:
```
https://physionet.org/content/ptb-xl/1.0.3/
```

**Model files** (`models/*.h5`, `*.tflite`, `*.pkl`) should be managed with Git LFS.
First-time setup:
```bash
git lfs install
...
To train the RF (Vital Signs) model:
```bash
