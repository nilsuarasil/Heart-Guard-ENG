...
import numpy as np
import pandas as pd          # Fix #6 – moved to module level
import matplotlib
...
# ─── Constants ────────────────────────────────────────────────────────────────
# Fix #3 – aligned with train_ptbxl_model.py output filename
...
def load_interpreter():
    # Model yükleyici yerine boş bir nesne döndürülüyor (TensorFlow kaldırıldığı için)
    return None
...
def detect_anomalies(signal, interp, fs):
    ...
    for start in range(0, len(signal) - WINDOW, STEP):
        ...
        # Sinyal anormalliğini (örneğin STEMI'deki ani yükselmeleri) tespit etmek için
        # bir skor üretiyoruz (0 ile 1 arasına sıkıştırılmış)
        score = (peak_val * 0.4) + (variance * 2.0)
        score = min(max(float(score), 0.0), 1.0)
        
        # Belirli aralıklarla yapay rastgelelik eklenebilir veya baz alınabilir
...
class HeartGuardDashboard(tk.Tk):
    def __init__(self):
        ...
        # Fix #5 – Models loaded once at startup with user-friendly error handling
        ...
        try:
            ...
            messagebox.showerror(
                "Model Not Found",
                f"Vital signs model not found:\n{RF_MODEL_PATH}\n\n"
                "Please train the model first:\n  python train_rf_model.py"
            )
        except Exception as exc:
            messagebox.showerror(
                "Model Load Error",
                f"An error occurred while loading the RF model:\n{exc}"
            )
    ...
    def _run_analysis(self):
        ...
        try:
            ...
            # Fix #4 – Use pre-loaded self.rf_model (double load removed)
            # Fix #6 – pandas now imported at module level
            self._log("\n--- Vital Signs Analysis ---", "head")
            rf = self.rf_model
            hr, sys_bp, dia_bp = generate_vitals()
            if rf is None:
                self._log("WARNING: RF model could not be loaded, skipping vital analysis.", "warn")
                rf_pred, rf_prob, rf_label, rf_color = 0, 0.0, "No Model", YELLOW
            else:
            ...
            for (rec_id, sampto, rec_title), ax in zip(chosen, ax_list):
                # Fix #7 – Offline fallback: network error → synthetic data
                ...
                try:
                    ...
                    self._log(
                        f"WARNING: MIT-BIH {rec_id} could not be downloaded "
                        f"({net_err.__class__.__name__}). "
                        f"Offline mode – using synthetic data.", "warn"
                    )
                    sig, t, fs = make_stemi()
                    rec_title = f"{rec_title} [SYNTHETIC – offline]"
